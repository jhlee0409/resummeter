...
- "업무 성과" → "[지표] X → Y로 개선 (N%)" (예: 전환율, 불량률, 처리시간, 매출, 만족도)
- "고객/사용자 응대" → "월 N건 처리, [달성률/만족도] N%"
- "반복 업무 개선" → "[프로세스] 개선으로 소요시간 N% 단축"`;

/** 경력기술서용 */
export const QUANTIFICATION_CAREER = `[수치화 패턴 예시]
- "업무 수행" → "N명/N건 대상 [업무] 수행, [성과 수치 기입]"
- "성과 개선" → "[지표] X → Y 개선 (N%)" (매출·전환율·불량률·처리시간·만족도 등)
- "프로젝트/업무 리드" → "N명 팀 리드, M개월간 [과제명] 완수"
이력서에 수치가 없으면 반드시 [수치 기입]을 사용하십시오.`;

/** 서술형용 */
export const QUANTIFICATION_NARRATIVE = `[수치화 패턴 예시]
- 처리량: "일 N건 → M건 처리" 또는 "[처리량 기입]"
- 효율: "처리시간/응답시간 X → Y 단축" 또는 "[효율 개선 수치 기입]"
